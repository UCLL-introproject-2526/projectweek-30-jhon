...
class Button(Entity):
    def __init__(self, x, y, main, text, color = (74, 39, 10), id=None, is_big=True, callback=None, is_animated=True):
    ...
    def is_clicked(self):
        ...
        if (self.x + 3 < mouse_pos[0] < self.x + self.width - 3 and
            ...
            if self.__is_animated:
                ...
                self.x = self.__save_x - (9 if self.__is_big else 3)
    ...
    def get_texture(self):
        ...
        surface = self.__get_img().copy()
        text_surface = font.render(self.__text, True, self.color)
